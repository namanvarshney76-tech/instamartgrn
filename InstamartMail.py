...
Downloads attachments with 'GRN' in filename from specified Gmail sender/search terms and uploads to Google Drive
"""
...
class GmailGDriveAutomation:
    def __init__(self, credentials_path: str, gdrive_folder_id: Optional[str] = None, attachment_filter: Optional[str] = None):
        ...
        Args:
            ...
            attachment_filter: Substring to filter attachments by filename (e.g., 'GRN')
        """
    ...
    def process_attachment(self, message_id: str, part: Dict, sender_info: Dict, 
        ...
        try:
            ...
            # Apply attachment filter for 'GRN'
            if self.attachment_filter and self.attachment_filter not in filename.lower():
                logger.info(f"[SKIPPED] Attachment {filename} does not contain '{self.attachment_filter}'")
                return False
...
def main():
    ...
    CONFIG = {
        'credentials_path': 'C:\\Users\\Lucifer\\Desktop\\New folder\\TBD\\GRN\\Instamart Automation\\credentials.json',
        'gdrive_folder_id': '141D679nCRsj3HM9wKhVWyxO9ni7-B6Ws',
        'sender': '',
        'search_term': 'grn & purchase return',
        'days_back':7,
        'max_results': 1000,
        'attachment_filter': 'GRN'  # Only process attachments containing 'GRN' in filename
    }
